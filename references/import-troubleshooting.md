...
| Snippet/app file input accepts no uploads, or type checkboxes unticked after import | Input field lacks `allowed_file_types` / `allowed_upload_methods` (tested: snippet import does not default them) | Add `allowed_file_types: [image, document]`, `allowed_upload_methods: [local_file, remote_url]`, `number_limits` to the input field | For file inputs always declare the upload settings explicitly |
